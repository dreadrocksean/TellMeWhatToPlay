...
import { TouchableHighlight, TouchableOpacity, Image, Text, Button, View } from 'react-native';
...
class SongItem extends Component {
  ...
  onDeleteSong() {
    this.props.onDeleteSong(this.props.song._id);
  }
      ...
      <Fragment>
        <View style={styles.leftInfo} >
          ...
          <View style={styles.info}>
            ...
            >{title}</AppText>
            <AppText
              textStyle={[styles.text, {color:'#999', fontSize: 11}]}
            >{artist}</AppText>
          </View>
        </View>
  ...
  render() {
    ...
    if (!showSong) { return null; }

    return (
      <View>
        <ListItem disabled={!visible || this.state.muted}>
          { this.state.isArtist
